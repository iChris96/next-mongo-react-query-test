...
const uptateTodo = async (req, res) => {
  const { todo } = req.body;
  const currentTodo = await Todos.findOneAndUpdate({id: todo._id}, { title: todo.title, isCompleted: todo.isCompleted});
  const updatedTodo = await Todos.findOne({id: todo._id});
